...
typedef struct MediaContext {
  ...
  AVIOContext *customIOContext;   // Custom IO context, when one is provided
  bool customIO;                  // True when customIOContext must be freed
  StreamDataContext
...
int IsBufferEmpty(
    const BufferState *state); // Check if the circular buffer is empty (returns
...
Buffer
LoadBuffer(int capacity); // Load a circular buffer with the specified capacity.
...
int WriteBuffer(Buffer *buffer, const uint8_t *srcData,
                int srcSize); // Write srcData to the buffer. Returns a negative
                              // value on error; otherwise, returns the actual
                              // size written (may be less than srcSize if not
                              // enough writable space is available).
...
PacketQueue
LoadQueue(int capacity); // Load a packet queue with the specified capacity.
...
AVPacket *
PeekPacket(const PacketQueue
               *queue); // Returns a pointer to the first available packet, or
                        // NULL if the queue is empty. Does not modify the queue
                        // or advance the read position.
...
int AVLoadCodecContext(StreamDataContext *streamCtx, const AVCodec *codec,
...
int SetMediaFlag(int flag, int value) {
  ...
  case MEDIA_VIDEO_QUEUE:
    MEDIA.videoQueueSize = MAX(value, 2);
    break;

  case MEDIA_AUDIO_QUEUE:
    MEDIA.audioQueueSize = MAX(value, 2);
    break;
  ...
  case MEDIA_AUDIO_FORMAT:
    ...
    if (!av_sample_fmt_is_planar(value)) {
      MEDIA.audioOutputFmt = (enum AVSampleFormat)CLAMP(value, AV_SAMPLE_FMT_U8,
                                                        AV_SAMPLE_FMT_DBL);
    } else {
...
MediaContext *LoadMediaContext(const char *fileName,
  ...
  if (!ctx) {
    TraceLog(LOG_ERROR, "MEDIA: Can't allocate MediaContext");
    return NULL;
  }
  ...
  *ctx = (MediaContext){0};
  ...
  if (streamReader.readFn) {
    ...
    ctx->customIOContext = avIOContext;
    ctx->customIO = true;
  ...
  if (!streamReader.readFn && !fileName) {
    TraceLog(LOG_ERROR, "MEDIA: A file name or stream reader is required");
    UnloadMediaContext(ctx);
    return NULL;
  }
  ...
  for (int i = 0; i < (int)ctx->formatContext->nb_streams; i++) {
    ...
    if (localCodecParameters->codec_type == AVMEDIA_TYPE_VIDEO &&
      ...
      if (ret == MEDIA_RET_SUCCEED) {
        ...
        const int videoOutputImageSize = av_image_get_buffer_size(
            AV_PIX_FMT_RGB24, codecCtx->width, codecCtx->height, 1);

        if (videoOutputImageSize <= 0) {
          TraceLog(LOG_ERROR,
                   "MEDIA: Invalid decoded frame buffer size: %i bytes.",
                   videoOutputImageSize);
        ...
        ctx->videoOutputImage.data = RL_MALLOC(videoOutputImageSize);
...
void UnloadMediaContext(MediaContext *ctx) {
  ...
  if (ctx->customIO && ctx->customIOContext) {
    av_freep(&ctx->customIOContext->buffer);
    avio_context_free(&ctx->customIOContext);
  }
...
int GetBufferWritableSpace(const BufferState *state) {
  ...
  return state->capacity - state->writePos + state->readPos - 1;
}
...
int GetBufferWritableSegmentSize(const BufferState *state) {
  ...
  return state->capacity - state->writePos - (state->readPos == 0 ? 1 : 0);
}
...
PacketQueue LoadQueue(int capacity) {
  ...
  if (capacity < 2) {
    TraceLog(LOG_WARNING,
             "MEDIA: Packet queue capacity must be at least 2; using 2.");
    capacity = 2;
  }
...
bool EnqueuePacket(PacketQueue *queue, AVPacket *src) {
  ...
  if (!queue->packets) {
    TraceLog(LOG_ERROR, "MEDIA: Trying to enqueue packet to a queue with no "
                        "allocated packets.");
    return false;
  }
...
bool DequeuePacket(PacketQueue *queue, AVPacket *dst) {
  ...
  if (!queue->packets) {
    TraceLog(LOG_ERROR, "MEDIA: Trying to dequeue packet from a queue with no "
                        "allocated packets.");
    return false;
  }
...
AVPacket *PeekPacket(const PacketQueue *queue) {
  ...
  if (!queue->packets) {
    TraceLog(LOG_ERROR, "MEDIA: Trying to peek a packet from a queue with no "
                        "allocated packets.");
    return NULL;
  }
...
int AVGrabPacket(MediaContext *ctx, int streamType, AVPacket *dst) {
  ...
  if (!ctx || streamType < 0 || streamType >= STREAM_COUNT) {
    TraceLog(LOG_ERROR, "MEDIA: Invalid stream type: %i", streamType);
...
int AVPeekPacket(MediaContext *ctx, int streamType, AVPacket **ptr) {
  ...
  if (!ctx || !ptr || streamType < 0 || streamType >= STREAM_COUNT) {
    TraceLog(LOG_ERROR, "MEDIA: Invalid packet peek request.");
    return MEDIA_ERR_GRAB_PACKET;
  }
  ...
  *ptr = NULL;
  ...
  if (!ctx->streams[streamType].codecCtx) {
    TraceLog(LOG_ERROR, "MEDIA: Trying to peek an unavailable stream: %i",
             streamType);
    return MEDIA_ERR_GRAB_PACKET;
  }
  ...
  if (IsQueueEmpty(queue)) {
    ...
    if (!EnqueuePacket(queue, ctx->avPacket)) {
      return MEDIA_ERR_GRAB_PACKET;
    }
  }
...
int AVDecodePacket(const MediaStream *media, int streamType,
                   const AVPacket *packet, bool discardPacket) {
  if (!media || !media->ctx || !packet || streamType < 0 ||
      streamType >= STREAM_COUNT) {
    TraceLog(LOG_ERROR, "MEDIA: Invalid packet decode request.");
    return MEDIA_ERR_UNKNOWN_STREAM;
  }

  const StreamDataContext *streamCtx = &media->ctx->streams[streamType];

  if (!streamCtx->codecCtx) {
    TraceLog(LOG_ERROR, "MEDIA: Trying to decode an unavailable stream: %i",
             streamType);
    return MEDIA_ERR_UNKNOWN_STREAM;
  }
...
int AVProcessVideoFrame(const MediaStream *media) {
  ...
  if (codec->width <= 0 || codec->height <= 0 || codec->width > INT_MAX / 3) {
    TraceLog(LOG_ERROR, "MEDIA: Invalid video dimensions: %d x %d",
...
int AVProcessAudioFrame(const MediaStream *media) {
  ...
  do {
    ...
    if (bytesPerFrame <= 0) {
      TraceLog(LOG_ERROR, "MEDIA: Invalid audio frame size.");
    ...
    if (writableSegmentSizeSamples <= 0) {
      TraceLog(LOG_WARNING,
               "MEDIA: Not enough contiguous space for decoding audio.");
      ret = MEDIA_ERR_OVERFLOW;
      break;
    }
