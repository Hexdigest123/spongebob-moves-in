...
typedef struct MediaStreamReader {
  int (*readFn)(void *userData, uint8_t *buffer,
                int bufferSize); // Custom read function pointer
  int64_t (*seekFn)(
      void *userData, int64_t offset,
      int whence); // Custom seek function pointer (optional, can be NULL)
  void *userData;  // Pointer to user-defined context, passed to the callback
                   // functions
} MediaStreamReader;
 ...
 */
typedef enum {
  MEDIA_IO_BUFFER,   // Size of the buffer used for media IO operations (only
                     // applies to custom read callbacks)
 ...
 * @param streamReader A valid MediaStreamReader with callback functions and
 * context
 * @param flags Combination of MediaLoadFlag values
 * @return MediaStream on success; empty structure on failure
 */
RLAPI MediaStream LoadMediaFromStream(MediaStreamReader streamReader,
                                      int flags);
 ...
 * @return Filled MediaProperties structure on success; empty structure on
 * failure
 */
